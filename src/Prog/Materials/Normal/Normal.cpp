...
namespace Raytracer
{
    ...
    Vec3 Normal::getColorAt(HitRecord record, [[maybe_unused]]Ray light, [[maybe_unused]]Vec3 lightColor) {
        // double intensity = normal.Dot(light.getDirection());
        Vec3 normalized = (record.normal+Vec3(1))* 0.5;
        return (record.normal+Vec3(1))* 0.5 * 255;
